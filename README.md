...
- 이름: 김채은 / Kim, Chaeeun
- 생년월일: 2003.04.15
- 소속: 한림대학교 정보과학대학 소프트웨어학부 콘텐츠IT 전공 / 빅데이터 복수전공 (4.06)
...
<div style="text-align: left;">
    ...
    <div style="margin: ; text-align: left;" "text-align: left;"> 
          ...
          <img src="https://img.shields.io/badge/Nextjs-000000?style=flat-square&logo=nextdotjs&logoColor=white"><br />
...
**👩🏻 김채은의 포트폴리오 🔎 _[깃허브](https://github.com/chaeeunkim30/portfolio.git)_**
- *2025.08.15 ~ 진행중*
- 개발자로서 지금까지 진행한 프로젝트를 소개하기 위해 웹 페이지를 기획하고 구현하고 있습니다. <br>HTML과 CSS, JS의 기본적인 개념을 탄탄하게 이해하고자 기획하였습니다.
...
- [한림대학교 - BIT융합연구소] PCR 반응 모니터링을 위한 소프트웨어
- PCR 반응 동안 생성되는 DNA의 양을 전기화학적으로 측정해 실시간으로 PCR 반응을 모니터링하기 위한 소프트웨어입니다.
- 기술 스택: C++, Qt프레임워크, WaveForms SDK, Analog Discovery2
- 구현 부분:
  - WaveForms SDK 연동
  - Impedance 계산하여 그래프를 띄워주는 기능
...
- 개발자들의 바닥까지 파헤치는 공간
- 커뮤니티를 통한 코드 질문과 오류 해결 / 스터디 모집 / 대회 및 부트캠프 공고 등 개발자들을 위한 다양한 정보를 모아둔 웹사이트입니다.
- 기술 스택: Figma, JavaScript, React, Node.js, SCSS, Firebase(Authentication, Firestore Database)
- 구현 부분:
  - 전반적인 개발 기획 주도 및 Figma를 이용한 웹 디자인
  - 재사용 가능한 컴포넌트 기반 로그인 및 회원가입 페이지 + Firebase 연동
  - 프로필 페이지(로그아웃, 프로필 이미지, 프로필 정보, 자기소개, 프로젝트 사진 및 소개)
  - 프로필 이미지 컴포넌트를 사용해 각 사용자의 댓글 마다 개개인의 프로필 이미지 표시 / 프로필 이미지 클릭 시 각 유저의 프로필 페이지로 이동
  - 본인이 작성한 댓글 수정 및 삭제 기능
...
- 사용자들이 더 나은 건강을 유지하며, 자신의 건강 상태에 맞는 영양제 복용을 선택하고 관리할 수 있는 실용적인 애플리케이션입니다.
- 기술 스택: Android, HTML, CSS, JAVA, Firebase
- 구현 부분:
  - 전반적인 개발 기획 주도
  - 애플리케이션의 전체 CSS 스타일 입히기
  - 인트로 화면, 로그인 및 회원가입 화면 + Firebase 연동
  - CalendarView를 이용한 캘린더 화면 및 Dialog를 통한 일정 추가 삭제 관리 기능 + Firebase 연동
  - 하단 네비게이션 바 구현
...
- 송/수신자가 사전에 서로 동일한 키를 공유(대칭키 암호 시스템)해야 하며, 임의로 만들어 놓은 알파벳 색상 표를 가지고 <br>알파벳에 맞는 색상과 키를 합성해 암호문을 만들 수 있는 시스템입니다.
<br>

**🐱 고양이 아케이드 퍼즐 게임 🔎 _[깃허브](https://github.com/chaeeunkim30/Basic-Of-VRAR-Game-Production.git)_**

- *2023.11.08 ~ 2023.11.28*
- 주인이 외출한 사이 고양이가 방 안을 돌아다니며 여기저기 흩어져 있는 고양이의 보물(먹이)을 찾아 <br>다양한 퀴즈를 풀며 보물을 얻는 아케이드 퍼즐 게임입니다.
- 기술 스택: Unity Engine, C#
- 구현 부분:
  - 전체 개발 기획 및 게임 디자인
  - 컨트롤러로 캐릭터 이동할 수 있는 Controller
  - 캐릭터 이동 시 시점 카메라가 캐릭터를 따라다닐 수 있도록 CameraMove
  - 아이템 클릭 시 퀴즈 + 정답 / 오답 로직 -> QuizController
  - 남은 보물, 남은 시간 / 게임 시작, 게임 종료, 다시하기를 위한 GameManager
...
- Arduino UNO Board와 서브모터, 발광 다이오드/LED/LCD, 버튼(스위치), 초음파 센서, <br>조도 센서, 부동 센서, 인체 감지 센서, 피에조 스피커를 이용해 제작했습니다.
- 기술 스택: Arduino IDE
- 구현 부분:
  - 개발 기획
